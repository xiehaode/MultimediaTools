...
// 先包含系统标准头文件，获取正确的结构体定义
#include <Windows.h>  // 包含BITMAPFILEHEADER、BITMAPINFOHEADER等定义
#include <d3d11.h>
#include <dxgi1_2.h>
#include <stdio.h>
#include <iostream>
...
// 屏幕捕获工具类
// 基于D3D11和DXGI实现高效的屏幕截图，支持保存为BMP格式
class ScreenCapture
{
public:
    ...
    bool CaptureScreenToBmp(std::string str) {
        // 获取屏幕尺寸
        ...
        for (int i = 0; i < 2; i++) {
            ...
            //保存为bmp格式
            char picNameB[128] = { 0 };
            snprintf(picNameB, sizeof(picNameB),str.c_str());
            if(i !=0)
...
// 快速捕获单张屏幕截图
inline bool QuickCaptureScreen(const std::string& filename)
{
    try
    {
        ScreenCapture capture;
        // 增加超时时间，确保能捕获到帧
        return capture.CaptureScreenToBmp(filename);
    }
    catch (const std::exception& e)
    {
        std::cerr << "快速捕获失败: " << e.what() << std::endl;
        return false;
    }
}
