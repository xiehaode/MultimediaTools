...
//            AvWorker_GetVideoFirstFrame(worker,"1.mp4","1.bmp",false);
            ...
            connect(openBtn, &QPushButton::clicked, this, [=]() {
                ...
                // 启动子进程（如果未启动）并发送播放指令
                m_ipcMgr->startChildProcess(mplayerPath, false); // 不隐藏主窗口
                
                // 给一点点时间让子进程连接（或者依靠基类的sendMessage缓存机制，如果实现了的话）
                // 这里我们简单处理，直接发送
                m_ipcMgr->sendMessage("play_video:" + videoPath);
                m_ipcMgr->activateWindow(); // 确保主窗口还在（虽然startChildProcess默认会处理）
            });
