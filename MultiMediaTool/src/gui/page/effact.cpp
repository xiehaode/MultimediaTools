...
void effact::on_pushButton_clicked()
{
    if (m_importPath.isEmpty()) {
        qDebug() << "未选择输入文件";
        QMessageBox::warning(this, "错误", "未选择输入文件，请先选择要转换的文件。");
        return;
    }

    if (!m_processor) {
        qDebug() << "处理器未初始化";
        QMessageBox::warning(this, "错误", "处理器未初始化，请重新启动程序或检查插件。");
        return;
    }
    ...
    // 使用 QFutureWatcher 进行异步处理，避免阻塞UI
    ...
    connect(watcher, &QFutureWatcher<int>::finished, this, [this, watcher, outPath, srcFormat, dstFormat]() {
        ...
        if (result == 0) {
            ...
            switch (result) {
                case -1:
                    errorMsg = "转封装失败：可能是文件格式不支持或文件损坏。\n建议：\n1. 检查输入文件是否完整\n2. 尝试转换为其他格式\n3. 使用ffmpeg命令行工具诊断";
                    break;
                case -2:
                    errorMsg = "参数错误：输入参数无效。";
                    break;
                case -3:
                    errorMsg = "内存不足：系统资源不够。";
                    break;
                case -4:
                    errorMsg = "编码器错误：视频参数不兼容。\n建议：检查视频分辨率是否为偶数。";
                    break;
                default:
                    errorMsg = QString("未知错误，错误码: %1\n建议：检查文件权限和磁盘空间").arg(result);
            }
            
            QMessageBox::critical(this, "转换失败", QString("%1\n\n%2 -> %3")
                                 .arg(errorMsg)
