...
void ffmpegCmd::initAIAssistant()
{
    ...
    if (!m_apiKey.isEmpty()) {
        ...
        if (m_aiChat) {
            updateAIStatus("AI助手已就绪", "#4CAF50");
            m_aiEnabled = true;
        } else {
            updateAIStatus("AI初始化失败", "#f44336");
            m_aiEnabled = false;
        }
    } else {
        updateAIStatus("请配置API密钥", "#FF9800");
        m_aiEnabled = false;
    }
...
void ffmpegCmd::on_btn_AISend_clicked()
{
    ...
    if (!m_aiEnabled || !m_aiChat) {
        appendAIChat("AI助手未启用或配置错误，请检查设置。", false);
        return;
    }
...
void ffmpegCmd::on_btn_AIClear_clicked()
{
    ...
    appendAIChat("对话已清空，可以开始新的对话。", false);
}
...
void ffmpegCmd::on_btn_AISettings_clicked()
{
    ...
    if (ok) {
        ...
        if (m_aiEnabled) {
            appendAIChat("API密钥已更新，AI助手重新初始化成功。", false);
        } else {
            appendAIChat("API密钥更新失败，请检查密钥是否正确。", false);
        }
    }
}
...
void ffmpegCmd::sendAIMessage(const QString &message)
{
    ...
    try {
        ...
        onAIResponseReceived("AI服务出现异常，请检查网络连接或API配置。");
    }
}
    ...
    if (regex.indexIn(response) != -1) {
        ...
        QTimer::singleShot(100, this, [this, ffmpegCmd]() {
            QMessageBox::StandardButton reply = QMessageBox::question(this,
                "发现FFmpeg命令",
                QString("AI回复中包含FFmpeg命令：\n\n%1\n\n是否要将此命令填入命令输入框？").arg(ffmpegCmd),
...
void ffmpegCmd::appendAIChat(const QString &message, bool isUser)
{
    ...
    if (isUser) {
        formattedMessage = QString("[%1] 你: %2").arg(timestamp, message);
    } else {
