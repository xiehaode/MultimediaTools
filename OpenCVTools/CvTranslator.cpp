...
cv::Mat CvTranslator::applyOilPainting(const cv::Mat& src, int radius, double sigma_color )
{
	// 1. 严格的参数校验，保证输入有效性
	if (src.empty()) {
		throw std::invalid_argument("输入图像为空，请检查Mat有效性");
	}
	if (src.channels() != 3) {
		throw std::invalid_argument("仅支持3通道彩色图像输入（BGR格式）");
	}
	if (src.depth() != CV_8U) {
		throw std::invalid_argument("仅支持8位无符号整型图像输入");
	}
	if (radius < 1) {
		throw std::invalid_argument("radius必须大于等于1");
	}
	if (sigma_color <= 0) {
		throw std::invalid_argument("sigma_color必须大于0");
	}
