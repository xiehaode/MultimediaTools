...
mpalyer::mpalyer(QWidget *parent) : QWidget(parent)
{
    ...
    connect(record, &QPushButton::clicked, this, [=]() {
        if (!p.isRecording()) {
            QString path = QFileDialog::getSaveFileName(this, "视频录制", "recorded.mp4", "视频文件(*.mp4)");
            if (!path.isEmpty()) {
                if (p.startRecord(path) == 0) {
                    record->setText("停止录制");
                    record->setStyleSheet("background-color: #D32F2F;"); // 红色表示录制中
                }
            }
        } else {
            p.stopRecord();
            record->setText("录制");
            record->setStyleSheet(""); // 恢复默认样式
        }
    });
